...
19. контроллер /api/v1/auth/request-verify-token возвращает null (`посмотреть, может стоит статус 200 выдавать, или json ответ какой нибудь, также с остальными которые указал ниже, на твое усмотрение`) и токен верификации с uuid пользователя в терминал (сообщение от user_manager)
